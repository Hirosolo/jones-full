...
class AddToCartRedirectView(View):
    def post(self, request):
        ...
        try:
            ...
            r = requests.get(f'https://fulfillnext.com/wp-json/wc/v3/products?sku={sku}')
            r.raise_for_status()
            data = r.json()
            if not data:
            ...
            if product.get('type') == 'variable':
                vres = requests.get(f'https://fulfillnext.com/wp-json/wc/v3/products/{product_id}/variations')
            ...
            if note:
                params += f'&note={note}'
            redirect_url = f'https://fulfillnext.com/{params}'
