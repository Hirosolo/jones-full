# path: myshop/api/views/wishlist/post.py
...
from myshop.models import WishList, Product
...
def action_to_wishlist(request):
    ...
    slug = data.get('slug') or request.GET.get('slug')

    # Ưu tiên tìm theo code, nếu không có thì tìm theo slug
    product = None
    if code:
        product = Product.objects.filter(code=code, is_available=True).first()
    elif slug:
