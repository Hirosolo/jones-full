"""
Replace fulfillnext.shop references in Product description fields
(desc / desc_short) with fulfillnext.com.

Three independent transforms (applied in order):

1. Literal logo home-link href: `href='https://fulfillnext.shop/'` →
   `href='https://fulfillnext.com/'` (single & double quote variants).
2. Literal WordPress logo image URL:
   `https://fulfillnext.shop/wp-content/uploads/2026/04/cropped-FulfillNext_logo-scaled-1.png`
   → `https://www.fulfillnext.com/images/fulfillnext_logo.png`.
3. Plain-text mentions: any `fulfillnext.shop` NOT followed by `/` →
   `fulfillnext.com`. Negative lookahead leaves URLs with paths
...
_IMG_OLD = (
    'https://fulfillnext.shop/wp-content/uploads/2026/04/'
    'cropped-FulfillNext_logo-scaled-1.png'
)
