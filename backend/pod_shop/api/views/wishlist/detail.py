# path: pod_shop/api/views/wishlist/detail.py
...
from pod_shop.models import Product, WishList
...
def check_item_wishlist(request):
    ...
    if product_code:
        product = Product.objects.filter(code=product_code, status='A').first()
    elif product_slug:
        product = Product.objects.filter(slug=product_slug, status='A').first()
    else:
        return Response({
            'ok': False,
