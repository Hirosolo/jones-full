...
| Record | Type | Value |
|--------|------|-------|
| `fulfillnext.com` (apex) | A | `<server IP>` |
| `www.fulfillnext.com` | A | `<server IP>` |
| `api.fulfillnext.com` | A | `<server IP>` |
...
```
/srv/fulfillnext/                          # repo clone về đây
...
| Biến | Ví dụ | Generate |
|------|-------|----------|
| `WEB_DOMAIN` | `www.fulfillnext.com` | — |
| `API_DOMAIN` | `api.fulfillnext.com` | — |
...
```bash
cd /srv/fulfillnext/deploy/single-server
...
curl -H "Host: www.fulfillnext.com"  http://localhost/
```
...
rsync -avz <old-server>:/srv/fulfillnext-api/deploy/hetzner/media-bucket/ \
           /srv/fulfillnext/deploy/single-server/media-bucket/
...
dig +short www.fulfillnext.com api.fulfillnext.com
...
docker compose run --rm certbot 'certbot certonly --webroot -w /var/www/certbot \
   --cert-name fulfillnext.com -d fulfillnext.com -d www.fulfillnext.com \
   --email <your-email> --agree-tos --non-interactive --no-eff-email'

docker compose run --rm certbot 'certbot certonly --webroot -w /var/www/certbot \
   --cert-name api.fulfillnext.com -d api.fulfillnext.com \
   --email <your-email> --agree-tos --non-interactive --no-eff-email'
...
```bash
curl -sI https://www.fulfillnext.com/  | grep "^HTTP"     # → HTTP/2 200
curl -sI https://api.fulfillnext.com/ | grep "^HTTP"
```
...
sudo crontab -e
# Thêm dòng:
0 3 * * * cd /srv/fulfillnext/deploy/single-server && docker compose run --rm certbot 'certbot renew' && docker compose exec nginx nginx -s reload
```
...
```bash
#!/usr/bin/env bash
# /srv/fulfillnext/scripts/daily-backup.sh
set -euo pipefail
cd /srv/fulfillnext/deploy/single-server
set -a; . ./.env; set +a

BACKUP_ROOT=/srv/fulfillnext/backups
TS=$(date +%Y%m%d-%H%M%S)
...
sudo crontab -e
# Thêm dòng:
0 2 * * * /srv/fulfillnext/scripts/daily-backup.sh
```
...
sudo crontab -e
# Chủ nhật 4 giờ sáng:
0 4 * * 0 rclone sync /srv/fulfillnext/backups/ remote:fulfillnext-backup/
```
...
- [ ] `curl -sI https://www.fulfillnext.com/` → `HTTP/2 200`
- [ ] `curl -sI https://api.fulfillnext.com/api/shop/brands-list/` → `HTTP/2 200`
...
bash /srv/fulfillnext/scripts/daily-backup.sh   # backup trước khi update
...
gunzip -c /srv/fulfillnext/backups/db/dump-YYYYMMDD-HHMMSS.sql.gz | \
  docker compose exec -T db psql -U "$POSTGRES_USER" -d "$POSTGRES_DB"
...
2. Thêm vhost nginx cho `www.fulfillnext.com` trỏ về `web:3000`.
