...
# Prereq: api.fulfillnext.com DNS already points to this server's IP.
set -euo pipefail
...
# Accept domain(s) as CLI args, default to api.fulfillnext.com
if [ "$#" -eq 0 ]; then
  DOMAINS=("api.fulfillnext.com")
else
  DOMAINS=("$@")
fi
PRIMARY="${DOMAINS[0]}"
