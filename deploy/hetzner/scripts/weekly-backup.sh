...
# Schedule via cron:  0 4 * * 0 /srv/fulfillnext-api/deploy/hetzner/scripts/weekly-backup.sh
set -euo pipefail
...
BACKUP_DIR="${BACKUP_DIR:-/srv/fulfillnext-api/backups}"
...
#   gsutil cp "$OUT" "gs://fulfillnext_cloudbuild/archive/$(basename "$OUT")"
# fi
