...
### What we are doing
Move the Django backend (`fulfillnext-api`) and PostgreSQL database from Google Cloud
...
- Audience is USA-based (`fulfillnext.com`, English content, target Los Angeles).
...
- **Cloud Run service**: `fulfillnext-api`, current revision `fulfillnext-api-00036-vtt`
- **Cloud Run image**: `asia-southeast1-docker.pkg.dev/fulfillnext/fulfillnext/fulfillnext-api:latest`
- **Cloud SQL instance**: `fulfillnext-db` (PostgreSQL 16, ENTERPRISE edition,
  `db-custom-1-3840`)
- **Cloud SQL connection string in Cloud Run env**:
  `postgres://fulfillnext:FulfillNext2026!@/fulfillnext?host=/cloudsql/fulfillnext:asia-southeast1:fulfillnext-db`
- **Domain mapping**: `api.fulfillnext.com` → `fulfillnext-api` via Cloud Run
  domain mapping (CNAME to `ghs.googlehosted.com.`)
- **Media bucket**: `gs://fulfillnext-media` (~35 MB, public-read)
- **GCP env vars** Cloud Run uses (full list to replicate on Hetzner):
  `DATABASE_URL`, `DEBUG=False`, `ALLOWED_HOSTS=*`,
  `SITE_URL=https://pezura.info`, `DJANGO_BASE_URL=https://api.pezura.info`,
  `CSRF_TRUSTED_ORIGINS`, `CORS_ALLOWED_ORIGINS`, `ADMIN_API_KEY`,
  `GS_BUCKET_NAME=fulfillnext-media`, `GS_DEFAULT_ACL=publicRead`.
...
- [ ] DNS provider for `api.fulfillnext.com` (Cloudflare? Vercel? Google
...
2. **U** — In Hetzner Cloud Console:
   - Project → create project `fulfillnext`.
   - Security → SSH Keys → upload public key (or generate `ssh-keygen -t ed25519 -f ~/.ssh/hetzner_fulfillnext`).
   - Servers → Add Server:
     - Location: **Frankfurt**
     - Image: **Ubuntu 24.04**
